...
df <- df %>%
  ...
  filter(!is.na(value), value >= 50, value <= 50000)
...
p <- ggplot(df, aes(x = Group, y = value, fill = Group)) +
  ...
  labs(
    x = "Group",
    y = "Duplication Length (bp)",
    title = "Duplication Length (Duplications 50-50,000 bp)"
  ) +
...
write_csv(export_table, "Duplications_50-50,000bp.csv")

# -------------------------------
# Save plot
# -------------------------------
ggsave(
  filename = "Duplications_50-50,000bp.tiff",
  plot = p,
